...
// src/canvas-extensions/body-link-edges-canvas-extension.ts
var BODY_EDGE_ID_PREFIX = "ble";
var BULLET_LINK_REGEX = /^\s*-\s*\[([^\]]+)\]\s*\[\[([^\]]+)\]\]/;
var BodyLinkEdgesCanvasExtension = class extends CanvasExtension {
  isEnabled() {
    return "bodyLinkEdgesFeatureEnabled";
  }
  init() {
    this.plugin.registerEvent(this.plugin.app.vault.on("modify", (file) => {
      for (const canvas of this.plugin.getCanvases())
    ...
    this.plugin.registerEvent(this.plugin.app.workspace.on(
      ...
      (canvas, edge) => this.onEdgeRemoved(canvas, edge)
    ));
  }
  async onFileModified(canvas, file) {
    var _a;
    for (const node of canvas.nodes.values()) {
      if (node.getData().type !== "file" || ((_a = node.file) == null ? void 0 : _a.path) !== file.path) continue;
  ...
  async onNodeChanged(canvas, _node) {
    for (const node of canvas.nodes.values()) {
      if (node.getData().type !== "file") continue;
      await this.updateBodyEdges(canvas, node);
    }
  }
  async onEdgeChanged(canvas, edge) {
    const edgeData = edge.getData();
  ...
  async onEdgeRemoved(canvas, edge) {
    var _a;
    const edgeData = edge.getData();
    const from = edge.from.node;
    const to = edge.to.node;
    ...
    if (!from.file || !to.file) return;
    const linktext = this.plugin.app.metadataCache.fileToLinktext(to.file, from.file.path);
    const line = `- [${(_a = edgeData.label) != null ? _a : ""}] [[${linktext}]]`;
    let content = await this.plugin.app.vault.cachedRead(from.file);
  ...
  async updateBodyEdges(canvas, node) {
  ...
  async getBodyEdges(canvas, node) {
    if (!node.file) return /* @__PURE__ */ new Map();
    const canvasFile = canvas.view.file;
    if (!canvasFile) return /* @__PURE__ */ new Map();
    ...
    for (const line of lines) {
      ...
      const targetNode = nodes.find((n) => {
        var _a;
        return n.id !== node.id && n.getData().type === "file" && ((_a = n.file) == null ? void 0 : _a.path) === (targetFile == null ? void 0 : targetFile.path);
      });
